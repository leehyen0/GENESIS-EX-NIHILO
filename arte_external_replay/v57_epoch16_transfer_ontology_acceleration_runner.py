...
def main(trigger,outdir):
    ...
    t=load(trigger); exp=t['parent_state_sha256']; st=restore_epoch15_parent(exp)
    assert t['epoch']==16 and st['epoch_completed']==15
    assert H({k:v for k,v in st.items() if k!='state_sha256'})==exp==st['state_sha256']
    assert any(x.get('operator_sha256')==PARENT_OP_SHA and x.get('target_consumed_for_generation') is False for x in st.get('generated_operator_registry',[]))
    ...
    j=int(hashlib.sha256(f'{rid}|{exp}|NAV_HELDOUT'.encode()).hexdigest()[:16],16)%len(eligible); held_idx=eligible[j]; held=xs[held_idx]
    dev=[e for i,e in enumerate(xs) if i!=held_idx][:120]
    ...
    pre={'selected_index':held_idx,'eligible_count':len(eligible),'input_sha256':hashlib.sha256(held['input'].encode()).hexdigest(),'prediction':pred,'wrong_prediction':wrong_pred,'selected_representation':full_name,'generated_abstraction_sha256':gen_sha,'full_search_cost':full_cost,'ablation_search_cost':abl_cost,'heldout_target_accessed':False}; pre_sha=H(pre)
    ...
    ns.setdefault('synergy_lineage',[]).append({'epoch':16,'github_run_id':rid,'parent_state_sha256':exp,'source_domain':'GENERIC_POSSESSION_EXCHANGE','target_domain':'NAVIGATE','e3_transfer_root':E3_TRANSFER_RECEIPT_SHA,'full_search_cost':full_cost,'ablation_search_cost':abl_cost,'success':ok,'wrong_control_success':wrong_ok,'generated_abstraction_sha256':gen_sha})
    ns.setdefault('synergy_credit',{})['TRANSFER_TO_ONTOLOGY_SEARCH_ACCELERATION']={'status':'SUPPORTED_BOUNDED' if ok and full_cost<abl_cost and not wrong_ok else 'NOT_SUPPORTED','full_cost':full_cost,'ablation_cost':abl_cost,'wrong_control_success':wrong_ok,'independent_custody':False}
    ...
    rec={'schema':'arte.transfer_to_ontology_search_acceleration_receipt/v57','epoch':16,'github_run_id':rid,'github_sha':os.environ.get('GITHUB_SHA'),'hosted_external':os.environ.get('GITHUB_ACTIONS')=='true','parent_state_sha256':exp,'child_state_sha256':ns['state_sha256'],'source_domain':'GENERIC_POSSESSION_EXCHANGE','target_domain':'NAVIGATE','source_owner':'google','source_repo':'BIG-bench','source_commit':BB_COMMIT,'heldout_target_hidden_until_after_prediction':True,'full_search_cost':full_cost,'ablation_search_cost':abl_cost,'wrong_control_success':wrong_ok,'actual_success':ok,'generated_abstraction_sha256':gen_sha,'e3_roots':t['e3_roots'],'claim_flags':FLAGS,'timestamp':datetime.now(timezone.utc).isoformat()}; rec['receipt_sha256']=H(rec); save(o/'epoch_receipt.json',rec)
