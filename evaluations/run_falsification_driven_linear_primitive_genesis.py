...
def proposal(descriptor: InterventionDescriptor) -> InterventionProposal:
    return InterventionProposal(
        ...
        reason="same authored semantics; raw channels require synthesized relation",
    )
...
def main(seed_path: str) -> None:
    ...
    # Complete single-channel threshold class (G5) is constructed before the
    # hidden G6 target is chosen. G6 skips all G5-equivalent signatures.
    ...
    g6_shadow = g6_engine.generate_novel(
        variables,
        descriptors,
        raw_observations,
        (),
    ...
    print(json.dumps({
        "status": "PASS_BOUNDED_FALSIFICATION_DRIVEN_MULTI_CHANNEL_LINEAR_PRIMITIVE_GENESIS_AND_DESCENDANT",
        ...
        "external_evaluator_selected_g6_generator": False,
        "verifierless_descendant_g6_version_space": len(no_verify_g6.compatible_model_ids),
