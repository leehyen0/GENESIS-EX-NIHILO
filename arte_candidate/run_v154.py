...
def main():
    ...
    artifact={
      ...
      'axis_providers':{'EXECUTION_AVAILABILITY':'vercel_deployment','SEMANTIC_CORRECTNESS':'github_actions_audited','FRESHNESS':'github_actions_audited','INDEPENDENCE_FIREWALL':'github_actions_audited','CLAIM_BOUNDARY':'github_actions_audited'},
      'provider_role_collapse':False,'source_dropout':False,'stale_epoch_mix':True,'forbidden_inferences_used':False,'candidate_can_modify_authority_product':False,
      'claim_boundary':{'AGI':False,'ASI':False,'independent_organization_custody':False,'recursive_acceleration_proven':False}}
    out=Path(a.output);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(artifact,indent=2)+'\n');print('V154_EPOCH_MIX_WRITTEN')
